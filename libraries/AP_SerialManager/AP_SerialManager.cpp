...
/**************************************************************************************************************
*函数原型：const AP_Param::GroupInfo AP_SerialManager::var_info[]
*函数功能：波特率初始化
*修改日期：2019-2-18
*修改作者：cihang_uav
*备注信息：函数初始化
...
/**************************************************************************************************************************************************
*函数原型：const AP_SerialManager::UARTState *AP_SerialManager::find_protocol_instance(enum SerialProtocol protocol, uint8_t instance) const
*函数功能：usb初始化
*修改日期：2019-2-18
*修改作者：cihang_uav
*备注信息： init_console - initialise console at default baud rate
...
/**************************************************************************************************************
*函数原型：AP_HAL::UARTDriver *AP_SerialManager::find_serial(enum SerialProtocol protocol, uint8_t instance) const
*函数功能：查找串口
*修改日期：2019-2-18
*修改作者：
*备注信息：find_serial - searches available serial ports for the first instance that allows the given protocol
...
/**************************************************************************************************************
*函数原型：uint32_t AP_SerialManager::find_baudrate(enum SerialProtocol protocol, uint8_t instance) const
*函数功能：查找波特率
*修改日期：2019-2-18
*修改作者：
*备注信息：find_baudrate - searches available serial ports for the first instance that allows the given protocol
          instance should be zero if searching for the first instance, 1 for the second, etc
...
/****************************************************************************************************************************************
*函数原型：bool AP_SerialManager::get_mavlink_channel(enum SerialProtocol protocol, uint8_t instance, mavlink_channel_t &mav_chan) const
*函数功能：获取mavlink协议通道
*修改日期：2019-2-18
*修改作者：
*备注信息：get_mavlink_channel - provides the mavlink channel associated with a given protocol
         instance should be zero if searching for the first instance, 1 for the second, etc
...
/***********************************************************************************************************************************
*函数原型：AP_SerialManager::SerialProtocol AP_SerialManager::get_mavlink_protocol(mavlink_channel_t mav_chan) const
*函数功能：
*修改日期：2019-2-18
*修改作者：
*备注信息：get_mavlink_protocol - provides the specific MAVLink protocol for a given channel, or SerialProtocol_None if not found
******************************************************************************************************************************/
AP_SerialManager::SerialProtocol AP_SerialManager::get_mavlink_protocol(mavlink_channel_t mav_chan) const
{
    ...
    for (uint8_t i=0; i<SERIALMANAGER_NUM_PORTS; i++)
    {
        if (state[i].protocol == SerialProtocol_MAVLink ||state[i].protocol == SerialProtocol_MAVLink2)
        {
...
/**************************************************************************************************************
*函数原型：void AP_SerialManager::set_blocking_writes_all(bool blocking)
*函数功能：设定块所有的写
*修改日期：2019-2-18
*修改作者：
*备注信息：set_blocking_writes_all - sets block_writes on or off for all serial channels
****************************************************************************************************************/
...
/********************************************************************************************************************************
*函数原型：uint32_t AP_SerialManager::map_baudrate(int32_t rate) const
*函数功能：映射波特率
*修改日期：2019-2-18
*修改作者：
*备注信息： map from a 16 bit EEPROM baud rate to a real baud rate. For PX4 we can do 1.5MBit, although 921600 is more reliable.
**********************************************************************************************************************************/
...
/**************************************************************************************************************
*函数原型：bool AP_SerialManager::protocol_match(enum SerialProtocol protocol1, enum SerialProtocol protocol2) const
*函数功能：串口匹配
*修改日期：2019-2-18
*修改作者：
*备注信息：protocol_match - returns true if the protocols match
****************************************************************************************************************/
